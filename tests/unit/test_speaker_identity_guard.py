...
class TestExtractVoiceprint:
    """Tests for extract_voiceprint()."""

    def test_returns_correct_shape(self, vocal_like, sr):
        emb = extract_voiceprint(vocal_like, sr)
        assert emb.ndim == 1
    ...
    def test_stereo_input(self, stereo_vocal, sr):
        emb = extract_voiceprint(stereo_vocal, sr)
        assert emb.shape[0] == N_MFCC * 3
        assert np.isfinite(emb).all()

    def test_sine_440(self, sine_440, sr):
        emb = extract_voiceprint(sine_440, sr)
        assert emb.shape[0] == N_MFCC * 3
        assert np.isfinite(emb).all()
    ...
    def test_short_audio(self, sr):
        ...
        emb = extract_voiceprint(short, sr)
        assert emb.shape[0] == N_MFCC * 3
        assert np.isfinite(emb).all()
...
class TestCosineSimilarity:
    ...
    def test_similar_embeddings(self, vocal_like, sr):
        ...
        emb1 = extract_voiceprint(vocal_like, sr)
        # Slightly attenuate
        emb2 = extract_voiceprint(vocal_like * 0.99, sr)
        sim = cosine_similarity(emb1, emb2)
        assert sim > 0.99

    def test_different_sounds(self, sine_440, sine_880, sr):
        """Different sine waves should differ."""
        emb1 = extract_voiceprint(sine_440, sr)
        emb2 = extract_voiceprint(sine_880, sr)
        sim = cosine_similarity(emb1, emb2)
...
class TestSpeakerIdentityGuard:
    ...
    def test_check_without_pre_embedding(self, vocal_like, sr):
        ...
        guard = SpeakerIdentityGuard()
        result = guard.check_phase("phase_19", vocal_like, sr)
        assert result.identity_preserved  # Graceful fallback
        assert "No pre-embedding" in result.warnings[0]
    ...
    def test_all_vocal_phases_listed(self):
        ...
        assert "phase_19" in VOCAL_PHASES
        assert "phase_42" in VOCAL_PHASES
        assert "phase_43" in VOCAL_PHASES
        assert "phase_65" in VOCAL_PHASES
    ...
    def test_multiple_checks_use_same_pre(self, vocal_like, sr):
        ...
        r1 = guard.check_phase("phase_19", vocal_like, sr)
    ...
    def test_custom_threshold(self, vocal_like, sr):
        ...
        result = guard.check_phase("phase_42", np.zeros_like(vocal_like), sr, threshold=0.0)
        assert result.identity_preserved
        ...
        result2 = guard.check_phase("phase_42", vocal_like, sr, threshold=1.0)
        assert result2.identity_preserved  # cos=1.0 >= 1.0 for identical inputs
