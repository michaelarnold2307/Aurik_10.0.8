        ...
        *,
        ...
        if artistic_intent is not None:
            ...
            imap.decision_log.append(
                f"ArtisticIntent: warmth={warmth:.2f} brill={brilliance:.2f} "
                f"dyn={preserve_dyn} risk={risk:.2f}"
            )
        ...
        if song_structure:
            for seg in song_structure:
                ...
                imap.per_segment.append(PerSegmentIntensity(
                    section_type=stype,
            ...
            imap.per_segment.append(PerSegmentIntensity(
                section_type="verse", start_sample=0, end_sample=len(mono)
            ))
        ...
        imap.global_modifiers["nr_global"] = float(np.clip(
            imap.global_modifiers.get("nr_global", 1.0), 0.1, 2.0
        ))

        self._last_map = imap
        logger.info("PIM: %d Bänder × %d Segmente — %d Entscheidungen",
                      len(imap.per_band), len(imap.per_segment), len(imap.decision_log))
        return imap
