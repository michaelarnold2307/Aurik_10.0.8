...
class DDIMSampler(object):
    ...
    def make_schedule(
        ...
        self.register_buffer(
            ...
            to_torch(np.sqrt(np.clip(1.0 - alphas_cumprod.cpu(), 1e-12, None))),
        )
        self.register_buffer(
            "log_one_minus_alphas_cumprod",
            to_torch(np.log(np.clip(1.0 - alphas_cumprod.cpu(), 1e-20, None))),
        )
        self.register_buffer(
            "sqrt_recip_alphas_cumprod",
            to_torch(np.sqrt(1.0 / np.clip(alphas_cumprod.cpu(), 1e-20, None))),
        )
        self.register_buffer(
            "sqrt_recipm1_alphas_cumprod",
            to_torch(np.sqrt(np.clip(1.0 / np.clip(alphas_cumprod.cpu(), 1e-20, None) - 1, 1e-20, None))),
        )
        ...
        self.register_buffer("ddim_sqrt_one_minus_alphas", np.sqrt(np.clip(1.0 - ddim_alphas, 1e-12, None)))
        sigmas_for_original_sampling_steps = ddim_eta * torch.sqrt(
            torch.clamp(1 - self.alphas_cumprod_prev, min=1e-12)
            / torch.clamp(1 - self.alphas_cumprod, min=1e-12)
            * torch.clamp(1 - self.alphas_cumprod / self.alphas_cumprod_prev, min=1e-12)
        )
        self.register_buffer(
    ...
    def p_sample_ddim(
        ...
        dir_xt = torch.clamp(1.0 - a_prev - sigma_t**2, min=1e-12).sqrt() * e_t
