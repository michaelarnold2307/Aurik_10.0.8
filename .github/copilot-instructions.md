...
>
> **instructions_version: 9.5** — SOTA-Science-Update Mai 2026 (v9.3) + Architektur-Update v9.12.9: RecordingChainProfiler (§2.66), Phase-Koalitions-Evaluation (§2.67), TemporalContinuityGuard (§2.69), RestorationMemory (§2.70), EraVocalProfile (§EraVocalProfile in musical_goals.instructions.md) + DefectScanner(54) + 62 CAUSES in CausalDefectReasoner + **Weltklasse-Qualitätsregeln v9.5**: Formant-Verschärfung (§2.71), Vibrato-Tiefe (§2.72), Noise-Textur-Invariante V19 (§NTI), Mikrodynamik-Korrelation V20 (§2.75), Mindestrauschboden V21, Pre-Echo-Prevention V22 (§2.73), Mono-Kompatibilität V23, Spektralfarbe V24 (§2.74), Wärmeband-Guard V25 (§2.76), Onset-Guard V26 (§2.77)
>
...
| Residualrauschen nach NR hat Material-fremdes Spektralprofil (Whitening) | `compute_noise_texture_distance(residual, material)` aus `backend/core/dsp/noise_texture_guard.py` nach jeder NR-Phase ≤ 0.25; Überschreitung → `nr_strength × 0.5` (WARNING — erhält Textur des Trägers; klinische Stille ist schlimmer als Rest-Rauschen) | V19 |
| Voiced-Frame-Mikrodynamik durch NR/Kompressor degradiert | Nach NR/Dynamics bei `panns_singing ≥ 0.25`: `frame_energy_correlation(pre, post, sr, frame_ms=10)` auf voiced-Zonen ≥ 0.97; Unterschreitung → Dry-Wet-Blend `wet = min(1.0, (corr - 0.90) / 0.07)` (§2.75) | V20 |
...
RestorationResult
> **Pipeline-Details** (§2.44–§2.77): [pipeline.instructions.md](instructions/pipeline.instructions.md)
...
| 1975–1990 | Vinyl, Cassette (Dolby B/C), Analogband | Dolby-Sättigungsrauschen, Dropouts, HF-Verlust; Dolby-AGC-Interaktion → Amplitude-Drift | phase_29, phase_24, phase_03, **phase_40** (wenn AMPLITUDE_DRIFT ≥ 0.30) | Cassette: **BW-Ceiling 12 kHz** (Type I) — keine Frequenzsynthese darüber; Dolby nur invertieren wenn NR-Schaltung aktiv war; `_MATERIAL_PHASE_FACTORS["cassette"]` Pflicht in `defect_phase_mapper.py` |
...
*Stand: Mai 2026 — Aurik 9.12.9 — instructions_version 9.5*
