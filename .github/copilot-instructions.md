...
| `_fast_goal_snapshot` Single-Segment-Bias: FFT-Basis auf einem einzigen Zentrum-Segment (`mono[N//2 …]`) | Multi-Segment-Mittelung (25 %/50 %/75 % des Songs): `_specs = [fft(seg25), fft(seg50), fft(seg75)]`; `_spec = mean(_specs)` — verhindert, dass ein einziger Akkord/Pausen-Frame alle spektralen Proxys auf ≤ 0.20 kollabieren lässt (§2.64 v9.12.2) | §2.64 |
| `authentizitaet`-Proxy auf ersten 8192 Samples: nach phase_24-Dropout-Repair kollabiert ACF-Peak von 0.71 → 0.06 | ACF auf Zentral-Drittel des Signals (`mono[N//3 : N//3+8192]`): vermeidet Intro/Dropout-reparierte Segmente, die andere Autokorrelationsprofile haben als vokal-periodische Abschnitte (§2.64 v9.12.2) | §2.64 |
| `transparenz`-Proxy: 5th/99th-Perzentile auf Zentrum-Segment → systematisch ≤ 0.20 für komprimiertes Pop/Schlager | `_p05_full + _p95_full` auf Vollsignal + Spectral-Flatness-Blend (3-Segment); Formel: `0.70 × log10(p95/p05)/4.0 + 0.30 × (1 − sfm_avg)` → repräsentativer Dynamikbereich (§2.64 v9.12.2) | §2.64 |
...
| LPC-Ordnung < 16 bei 16 kHz Analyse-SR oder < 30 bei 48 kHz | §4.4: LPC Ord. 30–40 @ 48 kHz nativ, ODER Downsampling → 16 kHz → LPC Ord. 16 (bevorzugt für Shellac BW ≤ 8 kHz). `lpc_formant_tracker.py` verwendet `_LPC_ORDER=16` + `_LPC_ANALYSIS_SR=16000`. | §4.4 |
| HNR-Guard fehlt nach NR-Phase auf Gesangsmaterial | `apply_hnr_blend(audio_pre, audio_post, sr)` aus `backend/core/dsp/hnr_guard.py` nach DFN/SGMSE+/OMLSA wenn `panns_singing ≥ 0.25`; ΔHNR > 3 dB → Dry-Blend (verhindert "klinischen" Klang) | §HNR |
| Passaggio-Sprung (Brust→Kopf) nicht geglättet → Timbre-Knick | `detect_vocal_register_temporal()` aus `backend/core/dsp/vocal_register_detector.py` statt per-Frame-Register; glättet Übergang ±5 Frames linear | §Passaggio |
| Cross-Segment Timbral Coherence nicht geprüft | `extract_song_noise_profile()` vor Pipeline, `compute_timbral_coherence_score()` nach NR-Phasen aus `backend/core/dsp/timbral_coherence_guard.py`; Kohärenz ≥ 0.80 Pflicht | §CSTC |
| Multi-Singer erkannt aber singer_identity_cosine-Gate aktiv | `detect_multi_singer(audio, sr, panns_singing)` aus `backend/core/dsp/vocal_register_detector.py`; wenn True: `metadata["multi_singer"]=True` + Resemblyzer-Gate überspringen (Embedding für Einzelidentität, nicht Duett/Chor) | §MultiSinger |
| SNR < 10 dB Gesangsmaterial ohne MIIPHER-Fallback | `get_miipher_plugin().enhance(audio, sr)` aus `plugins/miipher_plugin.py`; Stub→DeepFilterNet(−6 dB)→Wiener. Aktivierung: `should_activate(noise_snr_db, panns_singing)` | §4.4 |
| Shellac/WaxCylinder Formant-Tracking mit LPC-DSP statt DeepFormants | Kein direkter `get_lpc_formant_tracker().enhance()` in Phase_42; Shellac-Pfad: `get_formant_tracker().track()` aus `plugins/formant_tracker.py` (DeepFormants CNN primär, LPC-Burg Fallback intern) | §2.35c |
