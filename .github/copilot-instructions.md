...
| **Rauschboden** | CD-ähnlicher Export-Boden für alle analogen Tonträger; analoges Hiss-/Oberflächenrauschen wird als Defekt behandelt | ≤ −72 dBFS (moderner Standard) |
...
| Frontend-Version-Fallback als **hartcodierte Versionsnummer** statt `"unknown"` | `_AURIK_VERSION = "9.x.y"` als Fallback in `modern_window.py` bleibt veraltet sobald `__version__` weiterrückt → zeigt falsche Version im Fenstertitel wenn Import scheitert. **Richtig:** `_AURIK_VERSION = "unknown"` als Fallback — Import-Fehler wird sofort sichtbar. Frontend-Version-Invariante: alle 4 Pfade (`__init__.py`, `modern_window.py`, `splash_screen.py`, `main.py`) müssen konsistent sein (§FRONTEND_VERSION_INVARIANTE). Prüfung: `scripts/aurik_verboten_linter.py` V56-Regel greift wenn Fallback eine Versionsnummer enthält (regex `"9\.\d+\.\d+"`) | V56 |
| Neue additive Phase ohne `ForwardMaskingGuard` bei `panns_singing ≥ 0.25` | V41 abgedeckt für Phasen 06/07/23/26/37/38/39, aber **jede neue additive Phase** muss analog integriert werden. Erkennungsmuster: `panns_singing`-Kwarg vorhanden + `_effective_strength`-Berechnung + kein `get_forward_masking_guard()`-Aufruf → WARNING. Musterlösung: `phase_37_bass_enhancement.py` / `phase_39_air_band_enhancement.py` | V57 |
...
| Residualrauschen nach NR hat unkontrolliertes Spektralprofil (Whitening oder analoger Hiss-Rückfall) | `compute_noise_texture_distance(residual, material)` aus `backend/core/dsp/noise_texture_guard.py` nach jeder NR-Phase ≤ 0.25; bei klarem Gesang (`panns_singing ≥ 0.35`) gilt der strengere NTI-Schwellenwert **0.18**. Für analoge Tonträger ist das Export-Ziel CD-ähnliche Resttextur; Shellac/Vinyl/Tape/Kassette-Hiss darf nicht als Mindestboden zurückkehren. Überschreitung → `nr_strength × 0.5` (WARNING) | V19 |
...
| Pausenzone bekommt analogen Träger-Rauschboden zurück | `apply_noise_floor_minimum(audio, sr, material)` aus `backend/core/dsp/noise_floor_guard.py` darf für analoge Standardträger (`shellac`, `wax_cylinder`, `lacquer_disc`, `wire_recording`, `vinyl`, `tape`, `reel_tape`, `cassette`) keinen analogen Mindestboden injizieren. Falls Over-NR-Resttextur aufgefüllt werden muss, MUSS `restore_carrier_noise_texture()` CD-ähnliche Textur (`cd_digital`) und CD-ähnlichen Floor (≈ −74 dBFS, nie lauter als −68 dBFS im Testanker) nutzen. | V21 |
